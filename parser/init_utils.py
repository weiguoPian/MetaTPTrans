...
def node_dict_init(language):
    node_dic = dict()
    node_dic_path = os.path.join('{}/data'.format(root), language, 'node_vocab.json')
...
def read_files(language, type):
    ...
    if language == 'multi':
        language_types = ['python', 'go', 'javascript', 'ruby']
        for lt in language_types:
            lt_dir_path = os.path.join('{}/raw_data'.format(root), lt, type)
        ...
        dir_path = os.path.join('{}/raw_data'.format(root), language, type)
    ...
    for file in tqdm(file_list):
        if language == 'multi':
            f = open(file, 'r')
            # /raw_data/python/train/python_train_0.jsonl
            lt = file.split('raw_data/')[-1].split('/')[0]
        else:
        ...
        for line in lines:
            ...
            if language == 'multi':
                data = {'code': raw_data['code'], 'func_name': split_func_name(raw_data['func_name']), 'language': lt}
            else:
