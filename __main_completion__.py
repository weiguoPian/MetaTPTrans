...
def train():
    ...
    parser.add_argument("--accu_batch_size", type=int, default=256,
                        help="number of real batch_size per step, save gpu memory")
    parser.add_argument("--val_batch_size", type=int, default=512, help="number of batch_size of valid")
    parser.add_argument("--infer_batch_size", type=int, default=512, help="number of batch_size of infer")
    ...
    parser.add_argument("--meta_output", type=boolean_string, default=False, help="")
    ...
    if args.data_debug:
        train_dataset = CompletionPathAttenDataset(args, s_vocab, label_dict, type_='valid')
    else:
        train_dataset = CompletionPathAttenDataset(args, s_vocab, label_dict, type_='train')

    print("Loading Valid Dataset")
    valid_dataset = CompletionPathAttenDataset(args, s_vocab, label_dict, type_='valid')
    print("Loading Test Dataset")
    test_dataset = CompletionPathAttenDataset(args, s_vocab, label_dict, type_='test')
    if args.on_memory:
    ...
    if args.train:
        train_data_loader = DataLoader(train_dataset, batch_size=args.batch_size, num_workers=num_workers,
                                    shuffle=args.shuffle, collate_fn=completion_collect_fn)
    else:
